...
def _get_new_match_data(summoner):
    ...
    for match in matches_queryset:
        ...
        # Format damage numbers with commas for thousands
        damage_dealt_formatted = f"{main_participant.damage_dealt:,}"
        damage_taken_formatted = f"{main_participant.damage_taken:,}"
        gold_earned_formatted = f"{main_participant.gold_earned:,}"

        main_stats = {
            "kda": f"{kda:.2f}",
            "cs_min": f"{cs_min:.1f}",
            "damage_dealt": damage_dealt_formatted,
            "damage_taken": damage_taken_formatted,
            "gold_earned": gold_earned_formatted,
